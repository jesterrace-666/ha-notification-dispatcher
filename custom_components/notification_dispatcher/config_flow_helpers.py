"""Helper utilities for Notification Dispatcher config flow."""
...
_ALWAYS_WINDOW_ALIASES = {"24/7", "24h", "always", "immer", "ganztags"}
...
def _profile_schema(
    ...
    *,
    ...
    if profile_id:
        ...
        member_groups_default = _group_ids_from_input(profile.get(CONF_MEMBER_GROUPS, []))
    ...
    schema: dict[Any, Any] = {
        ...
        vol.Optional(
            CONF_ALWAYS_NOTIFY,
            default=bool(profile.get(CONF_ALWAYS_NOTIFY, False)),
        ): BooleanSelector(),
        vol.Optional(
            CONF_ONLY_WHEN_HOME,
    ...
    if include_time_windows:
        schema.update(_profile_schedule_schema_fields(profile))
...
def _profile_schedule_schema(profile: dict[str, Any] | None = None) -> vol.Schema:
    """Build the schedule-only schema for a profile."""
    return vol.Schema(_profile_schedule_schema_fields(profile or {}))


def _profile_schedule_schema_fields(profile: dict[str, Any]) -> dict[Any, Any]:
    """Build compact schedule fields for profile forms."""
    return {
        vol.Optional(
            CONF_WEEKDAY_WINDOW,
...
def _profile_from_user_input(
    ...
    weekday_window = _window_from_form_or_existing(
        user_input,
        existing,
        CONF_WEEKDAY_WINDOW,
        CONF_ALLOW_WEEKDAYS,
    ...
    weekend_window = _window_from_form_or_existing(
        user_input,
        existing,
        CONF_WEEKEND_WINDOW,
        CONF_ALLOW_WEEKENDS,
    ...
    return {
        ...
        CONF_ALWAYS_NOTIFY: always_notify,
...
def _window_from_form_or_existing(
    user_input: dict[str, Any],
    ...
    """Return window input, falling back to existing profile data."""
    if window_key in user_input:
        return user_input.get(window_key)
    if not existing:
        return None
    return _window_from_profile(
        existing,
        window_key,
        enabled_key,
        start_key,
...
def _normalize_time_window(value: Any, default: str, field: str) -> str:
    ...
    if not raw:
        return ""
    if raw.casefold() in _ALWAYS_WINDOW_ALIASES:
        return "00:00-23:59"
